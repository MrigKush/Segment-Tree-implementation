...
namespace RangeQueryForMultiplication
{
    class Program
    {
        static int[] st, ty;
        ...
        int RMQUtil(int ss, int se, int qs, int qe, int index)
        {
            ...
            int mid = (ss + se) / 2;
            return RMQUtil(ss, mid, qs, qe, 2 * index + 1)* RMQUtil(mid + 1, se, qs, qe, 2 * index + 2);
        }
        ...
        int updateValueUtil(int ss, int se, int i, int index, int diff)
        {
            ...
            if (se < i || ss > i)
                return st[index];
            int mid = (ss + se) / 2;
            st[index] = updateValueUtil(ss, mid, i, index * 2 + 1, diff)* updateValueUtil(mid + 1, se, i, index * 2 + 2, diff);
            return st[index];
        }
    }
}
